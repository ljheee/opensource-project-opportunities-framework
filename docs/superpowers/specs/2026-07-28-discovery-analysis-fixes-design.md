...
3. 停止条件（任一满足）：
   - 当前页最早样本早于 35 天前
   - 已翻页数达到上限（`sources.github.backfill_max_pages`，默认 30 页 = 最近 3000 个 star；命中上限说明近 30 天涨幅 >3000，velocity 必然饱和，缺失更早期数据无碍判断）
...
- 每日回溯项目数上限（`sources.github.backfill_max_per_day`，默认 100 个/天），避免首次对 712 个存量项目全量回溯时打爆 5000 次/小时的速率限额
